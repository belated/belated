...
_.each( tests, function( resultString, strFlags ) {
    ...
    _.each( filters, function( filter, filterLabel ) {
      ...
      _.each( { "string": strFlags, "object": objectFlags }, function( flags, flagsTypes ) {
        ...
        test( "belated.Callbacks( " + showFlags( flags ) + " ) - " + filterLabel, function() {
          ...
          output = "X";
          cblist = belated.Callbacks( flags );
...
test( "belated.Callbacks( options ) - options are copied", function() {
...
test( "jQuery.Callbacks.fireWith - arguments are copied", function() {
  ...
  var cb = belated.Callbacks( "memory" ),
    args = [ "hello" ];
...
test( "belated.Callbacks() - adding a string doesn't cause a stack overflow", function() {

  expect( 1 );

  belated.Callbacks().add( "hello world" );
